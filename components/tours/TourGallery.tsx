...
export default function TourGallery({ tour }: TourGalleryProps) {
  ...
  // Build image list: coverImage always first, then gallery extras (deduped)
  const allImages = [
    tour.coverImage,
    ...(tour.gallery ?? []).filter((g) => g && g !== tour.coverImage),
  ].filter(Boolean);

  const urls = allImages.map((filename) =>
    getTourImageUrl(tour.collectionId, tour.id, filename)
  );

  const prev = () => setLightbox((i) => (i === null ? null : (i - 1 + urls.length) % urls.length));
  const next = () => setLightbox((i) => (i === null ? null : (i + 1) % urls.length));

  // Single image — full width hero style
  if (urls.length <= 1) {
    return (
      <>
        <div
        ...
        <Lightbox urls={urls} current={lightbox} onClose={() => setLightbox(null)} onPrev={prev} onNext={next} name={tour.name} />
      </>
    );
  }
